...
passport.use(
	...
	new CustomStrategy(async function(req, done) {
		...
		if (guild) {
			...
			webhook = new InteractionWebhook(
				jsGay.client,
				jsGay.client.application.id,
				token
			);
		}
		...
		let body = `grant_type=authorization_code&client_id=Eyal282-Chess-ELO-Role-Bot-${
			jsGay.client.user.id
		}&redirect_uri=${myWebsite}${lichessEndOfWebsite}&code=${code}&code_verifier=${code_verifier}`;
		...
		if (result && result.id) {
			...
			if (guild) {
				...
				embed = new EmbedBuilder({
					description: `Successfully linked your [Lichess Profile](https://lichess.org/@/${
						result.id
					})`
				}).setColor(0x0099ff);
	...
	new CustomStrategy(async function(req, done) {
		...
		if (decryptedResult && decryptedResult.preferred_username) {
			...
			if (guild) {
				...
				embed = new EmbedBuilder({
					color: 0x0099ff,
					description: `Successfully linked your [Chess.com Profile](https://chess.com/member/${userName})`
				});
...
client.on('ready', async () => {
	...
	console.log(
		'\033[0;32mChess ELO Role has been completely loaded and is ready to use\nChess ELO Role has been completely loaded and is ready to use\nChess ELO Role has been completely loaded and is ready to use'
	);
...
for (const file of commandFiles) {
	...
	if (file.startsWith('mod-')) modCommands.push(command.data.name);

	client.commands.set(command.data.name, command);
}

commandFiles = fs
	.readdirSync('./commands-ephemeral')
	.filter(file => file.endsWith('.js'));
...
function deployGlobalSlashCommands() {
	...
	rest
		...
		.then(() =>
			console.log('Successfully unregistered guild application commands.')
		)
		.catch(console.error);
...
client.on('interactionCreate', async interaction => {
	...
	if (!command) return;
		...
		return interaction.reply({
			content: 'This bot does not accept DM Slash Commands',
			ephemeral: true
		});
	}
	...
	if (ephemeralCommands.indexOf(command) == -1) {
		...
		let bForbidden = await interaction.guild.commands.permissions.has({
			command: profileContextCommandId,
			permissionId: interaction.channel,
			permissionType: ApplicationCommandPermissionType.Channel
		});

		if (bForbidden) ephemeral = true;
...
client.on('interactionCreate', async interaction => {
	...
	if (!command) return;
		...
		return interaction.reply({
			content: 'This bot does not accept DM Context Menus',
			ephemeral: true
		});
	}
...
client.on('guildMemberAdd', async function(member) {
	...
	if (
		timestamp == undefined ||
		timestamp + 120 * 1000 < Date.now() ||
		(jsGay.isBotSelfHosted() && timestamp + 10 * 1000 < Date.now())
	) {
...
client.on('guildCreate', async function(guild) {
	...
	if (!jsGay.botHasBasicPermissionsByGuild(guild)) {
		...
		if (targetMember) {
			targetMember
				.send(
					`Bot needs the permissions of VIEW_CHANNELS, SEND_MESSAGES, READ_MESSAGE_HISTORY, MANAGE_ROLES to properly function.`
				)
				.catch(() => null);
		}
	}
...
client.on('interactionCreate', async interaction => {
	...
	if (
		...
		!interaction.customId.includes('retry-link')
	)
		return;

	let bLichess =
		interaction.message.embeds[0].url.includes('lichess.org') ||
		interaction.message.embeds[0].description.includes('lichess.org');

	let url = interaction.message.embeds[0].url.replace('\\', '/');
	...
	let [
		...
		titledRole,
		timeControlsBitwise
	] = await jsGay.getCriticalData(interaction);

	if (bLichess) {
		if (timestamp == undefined || timestamp + 10 * 1000 < Date.now()) {
			queue[`last-command-${interaction.user.id}`] = Date.now();

			let result = await fetch(`https://lichess.org/api/user/${username}`).then(
				response => {
					if (response.status == 404) {
						// Not Found
						return null;
			...
			if (result == null) {
				...
				let embed = new EmbedBuilder({
					...
					description: 'Rate Limit Encountered! Please try again!'
				}).setColor(0x0099ff);
				...
				let fullDiscordUsername =
					interaction.user.username + '#' + interaction.user.discriminator;

				if (
					result.profile &&
					((result.profile.location &&
						result.profile.location.includes(fullDiscordUsername)) ||
						(result.profile.bio &&
							result.profile.bio.includes(fullDiscordUsername)))
				) {
					...
					let embed = new EmbedBuilder({
						color: 0x0099ff,
						description: `Successfully linked your [Lichess Profile](${
							result.url
						})`
					});
					...
					let attachment = await jsGay.buildCanvasForLichess(
						interaction.user.username + '#' + interaction.user.discriminator
					);
					let embed = new EmbedBuilder({
						url: result.url,
						description:
							...
							'` in `Location` in your [Lichess Profile](https://lichess.org/account/profile)'
					}).setColor(0x0099ff);
		...
		if (timestamp == undefined || timestamp + 10 * 1000 < Date.now()) {
			...
			if (result == null) {
				...
				if (result.location && fullDiscordUsername == result.location) {
					result2 = await fetch(
						`https://api.chess.com/pub/player/${result.username}/stats`
					).then(response => {
						if (response.status == 404) {
							// Not Found
							return null;
					...
					] = Object.assign(result, result2); // stats are the most important thing!

					bUpdate = true;

					let embed = new EmbedBuilder({
						color: 0x0099ff,
						description: `Successfully linked your [Chess.com Profile](${
							result.url
						})`
					});
					...
					let attachment = await jsGay.buildCanvasForChessCom(
						interaction.user.username + '#' + interaction.user.discriminator
					);
					...
					let embed = new EmbedBuilder({
						...
						description:
							...
							'#' +
							interaction.user.discriminator +
							'` in `Location` in your [Chess.com Profile](https://www.chess.com/settings)'
					}).setColor(0x0099ff);
	...
	if (bUpdate) await jsGay.updateProfileDataByInteraction(interaction, true);
});
...
client.on('interactionCreate', async interaction => {
	...
	let obj = await jsGay.wipeDeletedRolesFromDB(
		interaction,
		ratingRoles,
		puzzleRatingRoles,
		titleRoles,
	...
	if (interaction.customId.startsWith('undo-unlink-lichess')) {
		...
		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Successfully linked your [Lichess Profile](https://lichess.org/@/${oldAccount})`
		});

		interaction
			.editReply({ embeds: [embed], failIfNotExists: false, ephemeral: true })
			.catch(() => null);
		...
		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Successfully linked your [Chess.com Profile](https://chess.com/member/${oldAccount})`
		});
		...
		queue[
			`last-lichess-code-verifier-of-${interaction.user.id}`
		] = code_verifier;
		queue[`last-lichess-interaction-token-of-${interaction.user.id}`] =
			interaction.token;
		queue[`last-link-guild-of-${interaction.user.id}`] = interaction.guild.id;

		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Sign in with Lichess by pressing the button below:`
		});
		...
		row = new ActionRowBuilder().addComponents(
			new ButtonBuilder()
				...
				.setURL(
					`https://lichess.org/oauth?response_type=code&redirect_uri=${myWebsite}${lichessEndOfWebsite}&code_challenge=${code_challenge}&code_challenge_method=S256&state=${state}&client_id=Eyal282-Chess-ELO-Role-Bot-${
						jsGay.client.user.id
					}`
				)
				.setStyle('Link')
		...
		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Sign in with Chess.com by pressing the button below:`
		});
		...
		row = new ActionRowBuilder().addComponents(
			new ButtonBuilder()
				...
				.setURL(
					`https://oauth.chess.com/authorize?client_id=${chessComClientId}&redirect_uri=${myWebsite}${chessComEndOfWebsite}&response_type=code&scope=openid%20profile&state=${state}&code_challenge=${code_challenge}&code_challenge_method=S256`
				)
				.setStyle('Link')
		);
		...
		queue[
			`last-chesscom-code-verifier-of-${interaction.user.id}`
		] = code_verifier;
		queue[`last-chesscom-interaction-token-of-${interaction.user.id}`] =
			interaction.token;
		queue[`last-link-guild-of-${interaction.user.id}`] = interaction.guild.id;
		...
		if (lichessAccount != null && lichessAccount != undefined) {
			...
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Successfully unlinked your Lichess Profile`
			});
			...
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Could not find a linked Lichess Profile`
			});
		}
		...
		if (lichessAccount != null && lichessAccount != undefined) {
			...
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Successfully unlinked your Chess.com Profile`
			});
			...
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Could not find a linked Chess.com Profile`
			});
		}
...
client.on('interactionCreate', async interaction => {
	...
	for (let i = 0; i < options.length; i++) {
		...
		if (!role) {
			embed = new EmbedBuilder({
				...
				description: `Error: role ${
					options[i].label
				} was not found, please recreate the menu!`
			});

			return interaction
				.reply({ embeds: [embed], failIfNotExists: false, ephemeral: true })
				.catch(() => null);
		} else if (!role.editable) {
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Error: I cannot give / remove the role <@&${
					role.id
				}> to other players. Please ensure the role is below my highest role`
			});
		...
		if (
			...
			embed = new EmbedBuilder({
				color: 0x0099ff,
				description: `Error: The role <@&${
					role.id
				}> has moderation capabilities, and cannot be given or taken`
			});
	...
	if (interaction.values.length == 0) {
		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Successfully removed all roles you have in this menu.`
		});
		...
		embed = new EmbedBuilder({
			...
			}>, and removed all unselected roles in the menu.`
		});
	} else {
		embed = new EmbedBuilder({
			color: 0x0099ff,
			description: `Successfully received all selected roles, and removed all unselected roles in the menu.`
		});
	}
