...
app.get('/fail', (req, res) =>
	res.sendFile('website/fail.html', { root: '.' })
);
...
let Constant_DefaultEmbedMessage =
	'Use the buttons below for linking your account to gain your rating roles!\n\nIf you want to link by editing your profile, you can still use /lichess and /chess';

let Constant_DefaultSelectUniqueRoleMessage =
	'Use the menu below to pick a unique role. Only one role may be picked.';

let Constant_DefaultSelectManyRolesMessage =
	'Use the menu below to pick up to {MAX_ROLES} unique roles. Every unpicked role is deleted.';
...
async function generateEmbedForProfileByInteraction(interaction) {
	...
	if (
		...
		timestamp1 + 120 * 1000 < Date.now() ||
		(isBotSelfHosted() && timestamp1 + 10 * 1000 < Date.now())
	) {
	...
	let [
		...
		puzzleRatingRoles,
		titleRoles,
		lichessRatingEquation,
		chessComRatingEquation,
		modRoles,
		timestamp,
		lichessAccount,
		chessComAccount,
		...
		titledRole,
		timeControlsBitwise
	] = await getCriticalData(interaction);

	let obj = await wipeDeletedRolesFromDB(
		interaction,
		ratingRoles,
		puzzleRatingRoles,
		titleRoles,
	...
	let authorObj = {
		...
		name: `${interaction.member.displayName}'s Profile`
	};
	let footerObj = {
	...
	if (result == null) description += ` Profile not linked.`;
	else {
		let emptyStr = '';
		...
		if (result.perfs) {
			if (result.perfs.correspondence)
				corresRating = emptyStr.concat(
					areBitsContained(timeControlsBitwise, Constant_CorresBitwise)
						? ''
						: ':x:',
					result.perfs.correspondence.rating.toString(),
					result.perfs.correspondence.prov == undefined ? '' : '(?)'
				);

			if (result.perfs.blitz)
				blitzRating = emptyStr.concat(
					areBitsContained(timeControlsBitwise, Constant_BlitzBitwise)
						? ''
						: ':x:',
					result.perfs.blitz.rating.toString(),
					result.perfs.blitz.prov == undefined ? '' : '(?)'
				);

			if (result.perfs.bullet)
				bulletRating = emptyStr.concat(
					areBitsContained(timeControlsBitwise, Constant_BulletBitwise)
						? ''
						: ':x:',
					result.perfs.bullet.rating.toString(),
					result.perfs.bullet.prov == undefined ? '' : '(?)'
				);

			if (result.perfs.rapid)
				rapidRating = emptyStr.concat(
					areBitsContained(timeControlsBitwise, Constant_RapidBitwise)
						? ''
						: ':x:',
					result.perfs.rapid.rating.toString(),
					result.perfs.rapid.prov == undefined ? '' : '(?)'
				);

			if (result.perfs.classical)
				classicalRating = emptyStr.concat(
					areBitsContained(timeControlsBitwise, Constant_ClassicalBitwise)
						? ''
						: ':x:',
					result.perfs.classical.rating.toString(),
					result.perfs.classical.prov == undefined ? '' : '(?)'
				);

			if (result.perfs.puzzle)
				puzzleRating = emptyStr.concat(
					result.perfs.puzzle.rating.toString(),
					result.perfs.puzzle.prov == undefined ? '' : '(?)'
				);
		}
		// Username
		description += `${
			result.patron ? ' <:lichess_patron:898198175633010708> ' : ''
		}${getEmojiFromTitle(result.title)}**[${result.username}](${
			result.url
		})**\n`;

		// Bullet
		description += `<:lichess_bullet:909072316019916850> Bullet: **${addStarForBestRating(
			highestRating,
			bulletRating,
			lichessRatingEquation
		)}** \\|\\| `;
		...
		// Blitz
		description += `<:lichess_blitz:909072315806003243> Blitz: **${addStarForBestRating(
			highestRating,
			blitzRating,
			lichessRatingEquation
		)}** \\|\\| `;
		...
		// Rapid
		description += `<:lichess_rapid:909072316128956476> Rapid: **${addStarForBestRating(
			highestRating,
			rapidRating,
			lichessRatingEquation
		)}**\n`;
		...
		// Classical
		description += `<:lichess_classical:909073486075527210> Classical: **${addStarForBestRating(
			highestRating,
			classicalRating,
			lichessRatingEquation
		)}** \\|\\| `;
		...
		// Daily
		description += `<:lichess_correspondence:909072696090976267> Daily: **${addStarForBestRating(
			highestRating,
			corresRating,
			lichessRatingEquation
		)}** \\|\\| `;
	...
	if (chessComAccountData == null) description += ` Profile not linked.`;
	else {
		let emptyStr = '';
		let bulletRating = 'Unrated';
		let blitzRating = 'Unrated';
		...
		if (result.chess_bullet)
			bulletRating = emptyStr.concat(
				areBitsContained(timeControlsBitwise, Constant_BulletBitwise)
					? ''
					: ':x:',
				result.chess_bullet.last.rating.toString(),
				result.chess_bullet.last.rd < Constant_ProvisionalRD ? '' : '(?)'
			);
		...
		if (result.chess_blitz)
			blitzRating = emptyStr.concat(
				areBitsContained(timeControlsBitwise, Constant_BlitzBitwise)
					? ''
					: ':x:',
				result.chess_blitz.last.rating.toString(),
				result.chess_blitz.last.rd < Constant_ProvisionalRD ? '' : '(?)'
			);
		...
		if (result.chess_rapid)
			rapidRating = emptyStr.concat(
				areBitsContained(timeControlsBitwise, Constant_RapidBitwise)
					? ''
					: ':x:',
				result.chess_rapid.last.rating.toString(),
				result.chess_rapid.last.rd < Constant_ProvisionalRD ? '' : '(?)'
			);
		...
		if (result.chess_daily)
			corresRating = emptyStr.concat(
				areBitsContained(timeControlsBitwise, Constant_CorresBitwise)
					? ''
					: ':x:',
				result.chess_daily.last.rating.toString(),
				result.chess_daily.last.rd < Constant_ProvisionalRD ? '' : '(?)'
			);

		if (
			result.tactics &&
			result.tactics.last &&
			result.tactics.last.rating &&
			result.tactics.last.rating != -1
		)
		...
		description += `${
			result.status == 'premium'
				? ` ${getEmojiFromPremiumLevel(result.membership_level)}`
				: ''
		}${getEmojiFromTitle(
			result.title
		)}**[${chessComAccount}](https://www.chess.com/member/${chessComAccount})**\n`;
...
// This returns the best rating AFTER formula.
// You can sneak a fake interaction if you assign .guild, .user and .member
async function updateProfileDataByInteractionsArray(
	interactions,
	useCacheOnly
) {
	interactions[0].isOriginalReal =
		typeof interactions[0].original.editReply !== 'undefined';

	if (
		!interactions[0].guild.members.me.permissions.has(
			PermissionsBitField.Flags.ManageRoles
		)
	) {
		if (interactions[0].isOriginalReal) {
			await interactions[0].original
				.editReply(
					`Could not assign roles. This is likely due to a permission issue. Bot's role must be lifted above rating roles, verified role, titled roles, etc...`
				)
				.catch(() => null);
			await interactions[0].original
				.followUp(
	...
	let [
		...
		titledRole,
		timeControlsBitwise
	] = await getCriticalData(interactions[0]);
	...
	for (let num = 0; num < interactions.length; num++) {
		...
		if (lichessAccount == undefined) {
			...
			if (useCacheOnly) {
				...
				result = await fetch(
					`https://lichess.org/api/user/${lichessAccount}`
				).then(response => {
		...
		if (result != null) {
			...
			if (result.perfs) {
				if (result.perfs.bullet && result.perfs.bullet.prov == undefined)
					bulletRating = result.perfs.bullet.rating;

				if (result.perfs.blitz && result.perfs.blitz.prov == undefined)
					blitzRating = result.perfs.blitz.rating;

				if (result.perfs.rapid && result.perfs.rapid.prov == undefined)
					rapidRating = result.perfs.rapid.rating;

				if (result.perfs.classical && result.perfs.classical.prov == undefined)
					classicalRating = result.perfs.classical.rating;

				if (
					result.perfs.correspondence &&
					result.perfs.correspondence.prov == undefined
				)
					corresRating = result.perfs.correspondence.rating;

				if (result.perfs.puzzle && result.perfs.puzzle.prov == undefined)
					puzzleRating = result.perfs.puzzle.rating;
			}
			...
			try {
				if (value != -1)
					value = Math.round(
						Parser.evaluate(lichessRatingEquation, { x: lichessHighestRating })
					);
			} catch {
				console.log(error);
			}
			...
			if (result.title) lichessTitle = result.title;
		}
		...
		if (chessComAccount == undefined) {
			...
			if (useCacheOnly) {
				...
				result = await fetch(
					`https://api.chess.com/pub/player/${chessComAccount}/stats`
				).then(response => {
					if (response.status == 404) {
						// Not Found
						return null;
		...
		if (result != null) {
			let result2 = await fetch(
				`https://api.chess.com/pub/player/${chessComAccount}`
			).then(response => {
				if (response.status == 404) {
					// Not Found
					return null;
			...
			let result3 = await fetch(
				`https://www.chess.com/callback/member/stats/${chessComAccount}`
			).then(response => {
				if (response.status == 404) {
					// Not Found
					return null;
			...
			let result4 = await fetch(
				`https://www.chess.com/callback/user/popup/${chessComAccount}`
			).then(response => {
				if (response.status == 404) {
					// Not Found
					return null;
			...
			if (result3 != null) {
				for (let i = 0; i < result3.stats.length; i++) {
					if (
						result3.stats[i].key == 'tactics' &&
						result3.stats[i].stats &&
						result3.stats[i].stats.rating
					) {
			...
			if (
				result4 != null &&
				result4.membership != undefined &&
				result4.membership.level != undefined
			) {
			...
			if (result2 != null) {
				...
				] = Object.assign(result2, result); // stats are the most important thing!
			} else {
				queue[
					`cached-chesscom-account-data-of-${interaction.user.id}`
				] = result;
			...
			if (
				...
				bulletRating = result.chess_bullet.last.rating;

			if (
				result.chess_blitz &&
				result.chess_blitz.last.rd < Constant_ProvisionalRD
			)
				blitzRating = result.chess_blitz.last.rating;

			if (
				result.chess_rapid &&
				result.chess_rapid.last.rd < Constant_ProvisionalRD
			)
				rapidRating = result.chess_rapid.last.rating;

			if (
				result.chess_daily &&
				result.chess_daily.last.rd < Constant_ProvisionalRD
			)
				corresRating = result.chess_daily.last.rating;

			if (result.tactics && result.tactics.highest)
				puzzleRating = result.tactics.last.rating;
			...
			let chessComHighestRating = Math.max(
				bulletRating,
				blitzRating,
				rapidRating,
				corresRating
			...
			try {
				if (value != -1)
					value = Math.round(
						Parser.evaluate(chessComRatingEquation, {
							x: chessComHighestRating
						})
					);
			} catch {}
			...
			if (result2 && result2.title) chessTitle = result2.title;
		}
		...
		if (fullRolesCache) {
			...
			for (let i = 0; i < ratingRoles.length; i++) {
				...
				if (index != -1) fullRolesArray.splice(index, 1);
			}
			...
			for (let i = 0; i < titleRoles.length; i++) {
				if (
					titleRoles[i].title == lichessTitle ||
					titleRoles[i].title == chessTitle
				)
			...
			if (highestRatingRole != null) fullRolesArray.push(highestRatingRole);
			...
			if (highestTitleRole != null) {
				...
				if (titledRole != undefined) fullRolesArray.push(titledRole);
			}

			if (
				verifyRole != null &&
				(lichessAccount != null || chessComAccount != null)
			)
			...
			if (
				fullRolesArray != Array.from(fullRolesCache.keys()) &&
				!(chessComAccount === undefined && lichessAccount === undefined)
			) {
...
function addEloCommand(message, ratingRoles, role, elo, guildRoles) {
	for (let i = 0; i < ratingRoles.length; i++) {
		if (ratingRoles[i].id == role.id) return undefined;
	}

	let highestBotRole = message.guild.members.resolve(client.user).roles.highest;
	let guildRole = guildRoles.get(role.id);

	if (highestBotRole.rawPosition < guildRole.rawPosition) return -1;
	else if (!isANumber(elo) && elo != -1) return -2;
...
function addPuzzleEloCommand(
	...
	for (let i = 0; i < puzzleRatingRoles.length; i++) {
		if (puzzleRatingRoles[i].id == role.id) return undefined;
	}
...
function addTitleCommand(message, titleRoles, role, title, guildRoles) {
	for (let i = 0; i < titleRoles.length; i++) {
		if (titleRoles[i].id == role.id) return undefined;
	}
...
function addModCommand(message, modRoles, role) {
	for (let i = 0; i < modRoles.length; i++) {
		if (modRoles[i].id == role.id) return undefined;
	}
...
async function isBotControlAdminByMessage(message, modRoles) {
	...
	for (let i = 0; i < modRoles.length; i++) {
		if (roleCache.has(modRoles[i])) return true;
	}
...
function botHasMessagingPermissionsByMessage(message) {
	let hasViewPermission = message.channel
		.permissionsFor(message.guild.me)
		.has(PermissionsBitField.Flags.ViewChannel, false);

	let hasMessageHistoryPermission = message.channel
		.permissionsFor(message.guild.me)
		.has(PermissionsBitField.Flags.ReadMessageHistory);

	let hasSendPermission = message.channel
		.permissionsFor(message.guild.me)
...
function botHasBasicPermissionsByGuild(guild) {
	let hasViewPermission = guild.members.me.permissions.has(
		PermissionsBitField.Flags.ViewChannel
	);

	let hasMessageHistoryPermission = guild.members.me.permissions.has(
		PermissionsBitField.Flags.ReadMessageHistory
	);

	let hasSendPermission = guild.members.me.permissions.has(
		PermissionsBitField.Flags.SendMessages
	);

	let hasManageRolesPermission = guild.members.me.permissions.has(
		PermissionsBitField.Flags.ManageRoles
	);
...
function replyAccessDeniedByMessage(message) {
	let embed = new EmbedBuilder({ description: `Access Denied` }).setColor(
		0x0099ff
	);
...
async function getCriticalData(interaction) {
	...
	let puzzleRatingRoles =
		manyMuch[`guild-puzzle-elo-roles-${interaction.guild.id}`];
	...
	let lichessRatingEquation =
		manyMuch[`guild-lichess-rating-equation-${interaction.guild.id}`];

	if (lichessRatingEquation == undefined) {
		lichessRatingEquation = Constant_lichessDefaultRatingEquation;
	}

	let chessComRatingEquation =
		manyMuch[`guild-chesscom-rating-equation-${interaction.guild.id}`];
	...
	let lichessAccountData =
		manyMuch[`cached-lichess-account-data-of-${interaction.user.id}`];
	let chessComAccountData =
		manyMuch[`cached-chesscom-account-data-of-${interaction.user.id}`];
	...
	let timeControlsBitwise =
		manyMuch[`guild-time-controls-${interaction.guild.id}`];

	if (timeControlsBitwise == undefined)
		timeControlsBitwise =
			Constant_BlitzBitwise |
			Constant_RapidBitwise |
			Constant_ClassicalBitwise |
			Constant_CorresBitwise;
...
async function wipeDeletedRolesFromDB(
	interaction,
	ratingRoles,
	puzzleRatingRoles,
	titleRoles,
	...
	await interaction.guild.roles
		...
		.then(async roles => {
			guildRoles = roles;
			let highestBotRole = interaction.guild.members.resolve(client.user).roles
				.highest;
			...
			if (highestBotRole) {
				for (let i = 0; i < ratingRoles.length; i++) {
					...
					if (
						role == undefined ||
						highestBotRole.rawPosition < role.rawPosition ||
	...
	ratingRoles.sort(function(a, b) {
		return a.rating - b.rating;
	});
	puzzleRatingRoles.sort(function(a, b) {
		return a.rating - b.rating;
	});
...
function getEmojiFromPremiumLevel(level) {
	...
	if (level == 50) return '<:chess_com_diamond:909056645131288606>';
	else if (level == 40) return '<:chess_com_platinum:931232604542353478>';
	else if (level == 30) return '<:chess_com_gold:931232604521373776>';

	return '';
}
function addStarForBestRating(highestRating, ratingToTest, ratingEquation) {
	if (!isANumber(ratingToTest)) return ratingToTest;
	...
	if (result != highestRating) return ratingToTest;
...
module.exports = {
	setModSlashCommands,
	generateEmbedForProfileByInteraction,
	updateProfileDataByInteraction,
	updateProfileDataByInteractionsArray,
	deleteMessageAfterTime,
	getRoleFromMentionString,
	addEloCommand,
	addPuzzleEloCommand,
	addTitleCommand,
	addModCommand,
	addCommandToHelp,
	isBotControlAdminByMessage,
	isBotControlAdminByInteraction,
	botHasMessagingPermissionsByMessage,
	botHasBasicPermissionsByGuild,
	botHasPermissionByGuild,
	replyAccessDeniedByMessage,
	replyAccessDeniedByInteraction,
	isBotSelfHosted,
	buildCanvasForLichess,
	buildCanvasForChessCom,
	getUserFullDiscordName,
	getCriticalData,
	wipeDeletedRolesFromDB,
	getBotIntegrationRoleByInteraction,
	getEmojiFromTitle,
	getEmojiFromPremiumLevel,
	addStarForBestRating,
	roleNamesToPurge,
	settings,
	client,
	app,
	...
	bootDate,
	areBitsContained,
	Constant_lichessDefaultRatingEquation,
	Constant_chessComDefaultRatingEquation,
	Constant_ProvisionalRD,
	Constant_Lichess,
	Constant_ChessCom,
	Constant_BulletBitwise,
	Constant_BlitzBitwise,
	Constant_RapidBitwise,
	Constant_ClassicalBitwise,
	Constant_CorresBitwise,
	Constant_DefaultEmbedMessage,
	Constant_DefaultSelectUniqueRoleMessage,
	Constant_DefaultSelectManyRolesMessage,
	titleList,
	getDebugChannel,
	hyperlinkBold
};
